...
class GameScene: SKScene {
    ...
    private func confiBackground() {
        for i in 0...1 {
            let backgroundSprite = SKSpriteNode(imageNamed: "dayScene-\(String(i))")
    ...
    private func configGroung() {
        for i in 0...1 {
            let backgroundSprite = SKSpriteNode(imageNamed: "dayGround-\(String(i))")
    ...
    private func addWolfAction(){
        ...
        let action = SKAction.animate(with: textureArray, timePerFrame: 0.05)
        wolf.run(SKAction.repeatForever(action))
        let runRight = SKAction.moveTo(x: size.width, duration: wolfSpeed)
        wolf.run(runRight)
    ...
    private func createStumpSprite() {
        stumpSprite = SKSpriteNode(texture: SKTexture(imageNamed: "stump"))
        let scale = 0.3
    ...
    private func addPhysicsBodyStump() {
        stumpSprite.physicsBody = SKPhysicsBody(texture: SKTexture(imageNamed: "stump"), size: CGSize(width: stumpSprite.size.width, height: stumpSprite.size.height))
    ...
    private func addStumpTo(_ sprite: SKSpriteNode) {
        ...
        let yPosition = frame.minY + sprite.size.height * 0.65
    ...
    private func rabbitJump() {
        ...
        let sounAction = SKAction.playSoundFileNamed("laugh.mp3", waitForCompletion: false)
        rabbit.run(sounAction)
    }
    ...
    private func createCarrotSprite() {
        carrotSprite = SKSpriteNode(texture: SKTexture(imageNamed: "carrot"))
        carrotSprite.name = "carrot"
        let scale = 0.1
    ...
    private func addPhysicsBodyCarrot() {
        carrotSprite.physicsBody = SKPhysicsBody(texture: SKTexture(imageNamed: "carrot"), size: carrotSprite.size)
    ...
    private func addCarrotTo(_ sprite: SKSpriteNode) {
        ...
        let yPosition = frame.minY + sprite.size.height * 1.2
        let xPosition = Double.random(in: minX...maxX)
