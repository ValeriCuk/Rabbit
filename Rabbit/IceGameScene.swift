...
class IceGameScene: SKScene {
    ...
    private func confiBackground() {
        for i in 0...1 {
            let backgroundSprite = SKSpriteNode(imageNamed: "iceScene-\(String(i))")
    ...
    private func configGroung() {
        for i in 0...1 {
            let backgroundSprite = SKSpriteNode(imageNamed: "iceGround-\(String(i))")
            backgroundSprite.size = CGSize(width: self.frame.width, height: self.frame.height * 0.25)
    ...
    private func goToGameOverScene(_ isWin: Bool){
        ...
        gameOverScene.level = .iceGameScene
    ...
    private func createStumpSprite() {
        stumpSprite = SKSpriteNode(texture: SKTexture(imageNamed: "snowman"))
        let scale = 0.23
    ...
    private func addPhysicsBodyStump() {
        stumpSprite.physicsBody = SKPhysicsBody(texture: SKTexture(imageNamed: "snowman"), size: CGSize(width: stumpSprite.size.width, height: stumpSprite.size.height))
    ...
    private func addStumpTo(_ sprite: SKSpriteNode) {
        ...
        let yPosition = frame.minY + sprite.size.height * 0.85
    ...
    private func rabbitJump() {
        ...
        let duration = 0.5
        let x = frame.size.width * 0.039
        let y = frame.size.height * 0.4
    ...
    private func createCarrotSprite() {
        carrotSprite = SKSpriteNode(texture: SKTexture(imageNamed: "cacao"))
        carrotSprite.name = "cacao"
        let scale = 0.15
    ...
    private func addPhysicsBodyCarrot() {
        carrotSprite.physicsBody = SKPhysicsBody(texture: SKTexture(imageNamed: "cacao"), size: carrotSprite.size)
    ...
    private func addCarrotTo(_ sprite: SKSpriteNode) {
        ...
        let yPosition = frame.minY + sprite.size.height * 1.7
        let xPosition = Double.random(in: minX...maxX)
...
extension IceGameScene: SKPhysicsContactDelegate {
