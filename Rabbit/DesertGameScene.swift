...
class DesertGameScene: SKScene {
    ...
    private func confiBackground() {
        for i in 0...1 {
            let backgroundSprite = SKSpriteNode(imageNamed: "desertScene-\(String(i))")
    ...
    private func configGroung() {
        for i in 0...1 {
            let backgroundSprite = SKSpriteNode(imageNamed: "desertGround-\(String(i))")
    ...
    private func goToGameOverScene(_ isWin: Bool){
        ...
        gameOverScene.level = .desertGameScene
    ...
    private func createStumpSprite() {
        stumpSprite = SKSpriteNode(texture: SKTexture(imageNamed: "cactus"))
        let scale = 0.3
    ...
    private func addPhysicsBodyStump() {
        stumpSprite.physicsBody = SKPhysicsBody(texture: SKTexture(imageNamed: "cactus"), size: CGSize(width: stumpSprite.size.width, height: stumpSprite.size.height))
    ...
    private func createCarrotSprite() {
        carrotSprite = SKSpriteNode(texture: SKTexture(imageNamed: "dragon-fruit"))
        carrotSprite.name = "dragon-fruit"
        let scale = 0.3
    ...
    private func addPhysicsBodyCarrot() {
        carrotSprite.physicsBody = SKPhysicsBody(texture: SKTexture(imageNamed: "dragon-fruit"), size: carrotSprite.size)
    ...
    private func addCarrotTo(_ sprite: SKSpriteNode) {
        ...
        let yPosition = frame.minY + sprite.size.height * 1.65
        let xPosition = Double.random(in: minX...maxX)
...
extension DesertGameScene: SKPhysicsContactDelegate {
