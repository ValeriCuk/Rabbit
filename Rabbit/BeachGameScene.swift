...
class BeachGameScene: SKScene {
    ...
    private func confiBackground() {
        for i in 0...1 {
            let backgroundSprite = SKSpriteNode(imageNamed: "beachScene-\(String(i))")
    ...
    private func configGroung() {
        for i in 0...1 {
            let backgroundSprite = SKSpriteNode(imageNamed: "beachGround-\(String(i))")
            backgroundSprite.size = CGSize(width: self.frame.width, height: self.frame.height * 0.2)
    ...
    private func goToGameOverScene(_ isWin: Bool){
        ...
        gameOverScene.level = .beachGameScene
    ...
    private func createStumpSprite() {
        stumpSprite = SKSpriteNode(texture: SKTexture(imageNamed: "ball"))
        let scale = 0.23
    ...
    private func addPhysicsBodyStump() {
        stumpSprite.physicsBody = SKPhysicsBody(texture: SKTexture(imageNamed: "ball"), size: CGSize(width: stumpSprite.size.width, height: stumpSprite.size.height))
    ...
    private func createCarrotSprite() {
        carrotSprite = SKSpriteNode(texture: SKTexture(imageNamed: "ice-cream"))
        carrotSprite.name = "ice-cream"
        let scale = 0.1
    ...
    private func addPhysicsBodyCarrot() {
        carrotSprite.physicsBody = SKPhysicsBody(texture: SKTexture(imageNamed: "ice-cream"), size: carrotSprite.size)
    ...
    private func addCarrotTo(_ sprite: SKSpriteNode) {
        ...
        let yPosition = frame.minY + sprite.size.height * 2.5
        let xPosition = Double.random(in: minX...maxX)
...
extension BeachGameScene: SKPhysicsContactDelegate {
